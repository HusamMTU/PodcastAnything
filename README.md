...
- `audio.mp3` (podcast audio; single voice or duo voices depending on mode/voice settings)
...
# edit .env and set required values before continuing
...
- `POLLY_DUO_VOICE_ID` (default `POLLY_VOICE_ID`; used as `HOST_B` voice when `script_mode=duo`)
...
- `ELEVENLABS_DUO_VOICE_ID` (default `ELEVENLABS_VOICE_ID`; used as `HOST_B` voice when `script_mode=duo`)
...
Duo-script with explicit voices:
...
curl -sS -X POST "$API_URL/executions" \
  ...
  -d '{"source_url":"https://en.wikipedia.org/wiki/Vision_transformer","style":"podcast","script_mode":"duo","voice_id":"Joanna","voice_id_b":"Matthew"}'
```
...
- `duo` mode is intended to be generated by the rewrite step prompt; unlabeled dialogue will fail audio synthesis.
...
- `script_mode=duo`
  - Script lines must be labeled with `HOST_A:` or `HOST_B:`
  - Audio generation alternates voices per speaker turn (`voice_id`/`voice_id_b` or provider defaults)
- `voice_id_b` is ignored in `single` mode and used only in `duo` mode.
...
- Duo audio fails with `script_mode=duo requires script lines prefixed...`
  - Ensure script lines are labeled `HOST_A:` / `HOST_B:` (the duo-script prompt enforces this).
